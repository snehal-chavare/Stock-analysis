...
const y = d3
  ...
  //.domain([50350 , 50400])
  .domain([d3.min(combinedParsed, d => Number(d.Low)), d3.max(combinedParsed, d => Number(d.High))])
  .nice()
...
svg
  ...
  .call(d3.axisBottom(x).ticks(d3.timeDay.every(3)).tickFormat(d3.timeFormat('%Y-%m-%d')))
  .selectAll('text')
...
.on("mouseover", (event, d) => {  // Add tooltip on hover
    tooltip.html(`Open: ${d.Open}<br>Close: ${d.Close}<br>High: ${d.High}<br>Low: ${d.Low}`)
        .style("visibility", "visible");
})
.on("mousemove", event => {  // Follow the mouse
