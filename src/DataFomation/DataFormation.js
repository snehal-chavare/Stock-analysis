...
const DataFormation = ({
  ...
  useEffect(() => {
    ...
    let data = fileContent[0].data.Time;
    ...
    const combinedResult = result.map((item) => {
      let index = fileContent[0].data["Time"].findIndex((time) =>
        time.includes(item)
      );
      return {
        Date: moment(fileContent[0].data["Time"][index]).format("YYYY-MM-DD"),
        Close: fileContent[0].data["c"][index],
        Open: fileContent[0].data["o"][index],
        High: fileContent[0].data["h"][index],
        Low: fileContent[0].data["l"][index],
      };
    });
